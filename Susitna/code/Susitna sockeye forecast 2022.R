...
#First re-create dataset:SusitnaRun2021 from sheet called "Stock-Specific Run Estimates" 
#in excel file: Susitna_Forecast_Analysis_2022_rdd.
#I must change to replace Escapement numbers with Spawners numbers, which from 
#"M-R Estimates". It is because Escapement numbers include PU and Sport Fish. 
#The original Escapement numbers are In-River Abundance.
#setwd("H:/Upper Cook Inlet/Forecasts_RB/Preseason Forecasts/2022/Susitna")
d=read.table("SusitnaRun2021.csv", sep = ",", skip = 1, header=T)
attach(d)
...
ave.ratio = mean(d$ratio, na.rm=T) #average of ratios(total_run/subtotal)
total.run<-subtotal*ave.ratio
f2023<-cbind(f.yr,r03,r12,r13,r22,r23, subtotal, total.run)
print(f2023, digits=5)
