...
#input data
setwd("H:/Upper Cook Inlet/Forecasts_RB/Preseason Forecasts/2022/Fish")
...
plot(log(R2_2) ~ log(R2_1), data=d,col="lightblue", pch=19, cex=2)
text(log(R2_2) ~ log(R2_1), data=d, labels=YEAR,cex=0.6, font=5)
fit.sibling <- lm(log(R2_2) ~ log(R2_1+1), data=d) 
fit.sibling2 <- lm((R2_2) ~ (R2_1), data=d) 
AIC(fit.sibling)
AIC(fit.sibling2)
